local ResourceManager = {} -- ResourceManager module

-- This module is used for retrieving, loading and unloading
-- of game resources (like images and music), it uses the
-- resource definitions module, as such one can just say
-- "Load the 'background' music" and this module will
-- use the 'background' definition atributtes (like the file path)
-- to load it.
...
function ResourceManager:load ( name )
  ...
  if not resourceDefinition then
    ...
    if (resourceDefinition.type == "image") then
      resource = self:loadImage (resourceDefinition)
    elseif (resourceDefinition.type == "tiled") then
      resource = self:loadTiledImage (resourceDefinition)
    elseif (resourceDefinition.type == "font") then
      resource = self:loadFont ( resourceDefinition )
    elseif (resourceDefinition.type == "sound") then
